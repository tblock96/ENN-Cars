...
class Network():
	...
	def activ_grad(self, output):
		return 1
	
	def backprop(self, input, answer):
		LEARNING_RATE = 0.05
		''' Gives the network an input with an expected output and uses the
		backpropagation algorithm to incrementally improve the weights'''
		node_vals = [[]]*len(self.nodes)
		node_vals[0] = input
		for i in range(len(self.weights)):
			node_vals[i+1] = np.dot(node_vals[i],self.weights[i])
			node_vals[i+1] = self.activation(node_vals[i+1])
		final = node_vals[-1]
		err = [0]*len(final)
		if len(final) != len(answer):
			print "Backprop Error: Dimensional mismatch"
			return
		# find delta for each node in the final layer
		delta = [[]]*(len(self.nodes))
		delta[-1] = [0]*self.nodes[-1]
		for i in range(self.nodes[-1]):
			err[i] = final[i]-answer[i]
			delta[-1][i] = err[i]*self.activ_grad(final[i])
		for j in range(2,len(self.nodes)+1):
			delta[-j] = [0]*self.nodes[-j]
			d = np.dot(self.weights[-j+1],delta[-j+1])
			for i in range(len(d)):
				delta[-j][i] = d[i]*self.activ_grad(node_vals[-j][i])
		for k in range(len(self.weights)):
			for j in range(self.nodes[k]):
				for i in range(self.nodes[k+1]):
					dEdw_ji = node_vals[k][j]*delta[k+1][i]
					self.weights[k][j][i] -= LEARNING_RATE*dEdw_ji
		sum = 0
		for e in err:
			sum += e**2
		return sum
