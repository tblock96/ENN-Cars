...
class Track(pg.sprite.Sprite):
 ...
 def draw_car(self, c):
  bx = c.location[0] - np.cos(c.theta)*c.length
  ex = c.location[0] + np.cos(c.theta)*c.length
  by = c.location[1] - np.sin(c.theta)*c.length
  ey = c.location[1] + np.sin(c.theta)*c.length
